...
#include "AbilitySystem/Abilities/Skill/Active/DarkMagician/OD_GA_Active_DarkMagician_001.h"
...
UOD_GA_Active_DarkMagician_001::UOD_GA_Active_DarkMagician_001()
	:Super()
{
	AbilityTags.AddTag(OD_Active_DarkMagician_001);
	
}

bool UOD_GA_Active_DarkMagician_001::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
...
void UOD_GA_Active_DarkMagician_001::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	auto* MontageTask = UAbilityTask_PlayMontageAndWait::CreatePlayMontageAndWaitProxy(
		...
		AbilityMontage,
		MontageRate);
	MontageTask->OnCompleted.AddDynamic(this,&UOD_GA_Active_DarkMagician_001::OnMontageComplete);
	MontageTask->OnCancelled.AddDynamic(this,&UOD_GA_Active_DarkMagician_001::OnMontageCancelled);
	MontageTask->OnInterrupted.AddDynamic(this,&UOD_GA_Active_DarkMagician_001::OnMontageInterrupted);
	MontageTask->OnBlendOut.AddDynamic(this,&UOD_GA_Active_DarkMagician_001::OnMontageBlendOut);
	...
	if (K2_HasAuthority())
	{
		...
		auto* WaitHitTask = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(
			...
			false,
			true);
		WaitHitTask->EventReceived.AddDynamic(this, &UOD_GA_Active_DarkMagician_001::OnHitNotifyEvent);
	...
	auto* WaitRemoveTask = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(
		...
		true,
		true);
	WaitRemoveTask->EventReceived.AddDynamic(this, &UOD_GA_Active_DarkMagician_001::OnRemoveNotifyEvent);
...
void UOD_GA_Active_DarkMagician_001::EndAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
...
void UOD_GA_Active_DarkMagician_001::OnMontageComplete()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_DarkMagician_001::OnMontageCancelled()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_DarkMagician_001::OnMontageInterrupted()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_DarkMagician_001::OnMontageBlendOut()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_DarkMagician_001::OnSpawnSuccess(AActor* SpawnedActor)
{
}

void UOD_GA_Active_DarkMagician_001::OnSpawnFail(AActor* SpawnedActor)
{
}

void UOD_GA_Active_DarkMagician_001::OnHitNotifyEvent(const FGameplayEventData InData)
{
	...
	Task->Success.AddDynamic(this, &UOD_GA_Active_DarkMagician_001::OnSpawnSuccess);
	Task->DidNotSpawn.AddDynamic(this, &UOD_GA_Active_DarkMagician_001::OnSpawnFail);
	...
	if (Task->BeginSpawningActor(this, TDH, AbilitySpawnActor, SpawnedActor))
	{
		if (AODProjectileActor* Projectile = Cast<AODProjectileActor>(SpawnedActor))
		{
			...
			if (AODCombatCharacter* Owner = Cast<AODCombatCharacter>(GetOwner))
			{
				...
				//Spawn Transform
				FVector ForwardLoc = Owner->GetActorLocation() + Owner->GetActorForwardVector() * 60.f;
...
void UOD_GA_Active_DarkMagician_001::OnRemoveNotifyEvent(const FGameplayEventData InData)
{
