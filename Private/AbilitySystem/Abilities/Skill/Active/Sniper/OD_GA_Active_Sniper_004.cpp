...
#include "AbilitySystem/Abilities/Skill/Active/Sniper/OD_GA_Active_Sniper_004.h"
...
UOD_GA_Active_Sniper_004::UOD_GA_Active_Sniper_004()
	:Super()
{
	AbilityTags.AddTag(OD_Active_Sniper_004);
	SkillProjectileParms = FODSkillProjectileParms(3000.f);
}

bool UOD_GA_Active_Sniper_004::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
...
void UOD_GA_Active_Sniper_004::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	auto* MontageTask = UAbilityTask_PlayMontageAndWait::CreatePlayMontageAndWaitProxy(
		...
		AbilityMontage,
		MontageRate);
	MontageTask->OnCompleted.AddDynamic(this,&UOD_GA_Active_Sniper_004::OnMontageComplete);
	MontageTask->OnCancelled.AddDynamic(this,&UOD_GA_Active_Sniper_004::OnMontageCancelled);
	MontageTask->OnInterrupted.AddDynamic(this,&UOD_GA_Active_Sniper_004::OnMontageInterrupted);
	MontageTask->OnBlendOut.AddDynamic(this,&UOD_GA_Active_Sniper_004::OnMontageBlendOut);
	...
	if (K2_HasAuthority())
	{
		...
		auto* WaitHitTask = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(
			...
			false,
			true);
		WaitHitTask->EventReceived.AddDynamic(this, &UOD_GA_Active_Sniper_004::OnHitNotifyEvent);
	...
	auto* WaitRemoveTask = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(
		...
		true,
		true);
	WaitRemoveTask->EventReceived.AddDynamic(this, &UOD_GA_Active_Sniper_004::OnRemoveNotifyEvent);
...
void UOD_GA_Active_Sniper_004::EndAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
...
void UOD_GA_Active_Sniper_004::OnMontageComplete()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_Sniper_004::OnMontageCancelled()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_Sniper_004::OnMontageInterrupted()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_Sniper_004::OnMontageBlendOut()
{
	EndAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true, false);
}

void UOD_GA_Active_Sniper_004::OnSpawnSuccess(AActor* SpawnedActor)
{
}

void UOD_GA_Active_Sniper_004::OnSpawnFail(AActor* SpawnedActor)
{
}

void UOD_GA_Active_Sniper_004::OnHitNotifyEvent(const FGameplayEventData InData)
{
	...
	Task->Success.AddDynamic(this, &UOD_GA_Active_Sniper_004::OnSpawnSuccess);
	Task->DidNotSpawn.AddDynamic(this, &UOD_GA_Active_Sniper_004::OnSpawnFail);
	...
	if (Task->BeginSpawningActor(this, TDH, AbilitySpawnActor, SpawnedActor))
	{
		if (AODProjectileActor* Projectile = Cast<AODProjectileActor>(SpawnedActor))
		{
			...
			if (AODCombatCharacter* Owner = Cast<AODCombatCharacter>(GetOwner))
			{
				...
				//Spawn Transform
				FVector ForwardLoc =  Owner->GetActorLocation() + Owner->GetActorForwardVector() * 70.f; 
...
void UOD_GA_Active_Sniper_004::OnRemoveNotifyEvent(const FGameplayEventData InData)
{
